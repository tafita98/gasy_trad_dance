...
function DemandeDonwloadSansVali() {
    const { id } = useParams();
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(errors).length === 0) {
            setLoading(true);
            try {
                const { data } = await axios.post(`/api/EnvoieDonneEmail`, { microdatas_id: id, email: email}, {
                    headers: {
                        'Authorization': `Bearer ${user.token}`
                    }
                });
...
export default DemandeDonwloadSansVali;
